	...
	describe('Node edge addition / query / removal tests - ', () => {
		...
		describe('a little more complex scenario - ', () => {
			...
			describe('previous, next, connected and adjacent nodes', () => {
				...
				test('should find nodes c and d as previous nodes', () => {
					...
					expect(prevs).toEqual(expect.arrayContaining([n_a]));
					expect(prevs).toEqual(expect.arrayContaining([n_c]));
					expect(prevs).toEqual(expect.arrayContaining([n_d]));		
				});
				...
				test('should find nodes a, b and d as next nodes', () => {
					...
					expect(nexts).toEqual(expect.arrayContaining([n_c]));
					expect(nexts).toEqual(expect.arrayContaining([n_a]));
					expect(nexts).toEqual(expect.arrayContaining([n_b]));
					expect(nexts).toEqual(expect.arrayContaining([n_d]));	
				});
				...
				test('should find nodes b and c as connected (undirected) nodes', () => {
					...
					expect(conns).toEqual(expect.arrayContaining([n_a]));
					expect(conns).toEqual(expect.arrayContaining([n_b]));
					expect(conns).toEqual(expect.arrayContaining([n_c]));
				});
				...
				test('should find nodes a, b, c and d as adjacent (reachable) nodes', () => {
					...
					expect(adjs).toEqual(expect.arrayContaining([n_a]));
					expect(adjs).toEqual(expect.arrayContaining([n_b]));
					expect(adjs).toEqual(expect.arrayContaining([n_c]));	
					expect(adjs).toEqual(expect.arrayContaining([n_d]));
				});
				...
				test('should find node a as undirected neighbor node from node b', () => {
					...
					expect(conns).toEqual(expect.arrayContaining([n_b]));
					expect(conns).toEqual(expect.arrayContaining([n_c]));	
					expect(conns).toEqual(expect.arrayContaining([n_d]));
				});
				...
				test('should find node a as directed previous neighbor from node b', () => {
					...
					expect(prevs).toEqual(expect.arrayContaining([n_a]));
					expect(prevs).toEqual(expect.arrayContaining([n_b]));
					expect(prevs).toEqual(expect.arrayContaining([n_c]));	
					expect(prevs).toEqual(expect.arrayContaining([n_d]));
				});
			});			
			...
			describe('deletion of single edges by type', () => {
				...
				beforeEach(
                    'reinstantiate the "more complex scenario" from above, plus one more loop',
                    () => {
        ...
        test('should throw an error upon trying to remove an unconnected edge', () => {
          expect(n_a.removeEdgeID.bind(n_a, 'menotexists')).toThrowError('Cannot remove unconnected edge.');
				...
				test(
                    'should confirm our expectations about node degrees in this example',
                    () => {	
