...
import { GraphMode, GraphStats, MinAdjacencyListDict, MinAdjacencyListArray, NextArray } from "@/core/interfaces";
...
describe("GRAPH TESTS: ", () => {
  ...
  describe("Basic graph instantiation - ", () => {
    ...
    test("should correctly instantiate a graph with GraphMode INIT (no edges added)", () => {
      graph = new Graph("Test graph");
    ...
    describe("adding nodes and edges -", () => {
      ...
      test("should correctly clone and add a node from another graph without adding its edges", () => {
        ...
        expect(clone_graph.getEdgeById.bind(clone_graph, "1")).toThrowError(
          "cannot retrieve edge with non-existing ID."
        );
      ...
      test("should refuse to add an edge if one of the nodes does not exist in the graph", () => {
        node_a = new $N.BaseNode("floating_node_a");
        node_b = graph.addNodeByID("B_in_graph");
        expect(graph.addEdgeByID.bind(graph, "edge_to_nirvana", node_a, node_b)).toThrowError(
          "can only add edge between two nodes existing in graph"
        );
      });
      ...
      test("should refuse to add an edge if node A does not exist", () => {
        graph = new Graph("Test graph");
        expect(graph.addEdgeByNodeIDs.bind(graph, "dontaddme", "A", "B")).toThrowError(
          "Cannot add edge. Node A does not exist"
        );
      });
      ...
      test("should refuse to add an edge if node B does not exist", () => {
        ...
        node_a = graph.addNodeByID("A");
        expect(graph.addEdgeByNodeIDs.bind(graph, "dontaddme", "A", "B")).toThrowError(
          "Cannot add edge. Node B does not exist"
        );
      });
      ...
      test("should check if given node IDs are just coincidentally present in the graph but point to invalid node objects", () => {
        node_a = graph.addNodeByID("A");
        node_b = graph.addNodeByID("B");
        ...
        expect(clone_graph.addEdge.bind(clone_graph, edge_ab)).toThrowError(
          "can only add edge between two nodes existing in graph"
        );
      });

      test("should check if just one (A) of the given node IDs is just coincidentally present in the graph but points to an invalid node object", () => {
        node_a = graph.addNodeByID("A");
        node_b = graph.addNodeByID("B");
        edge_ab = graph.addEdgeByID("1", node_a, node_b);
      ...
      test("should check if just one (B) of the given node IDs is just coincidentally present in the graph but points to an invalid node object", () => {
        node_a = graph.addNodeByID("A");
        node_b = graph.addNodeByID("B");
        edge_ab = graph.addEdgeByID("1", node_a, node_b);
        let new_node_a = clone_graph.addNodeByID("A");
  ...
  describe("finding nodes and edges by ID and Label", () => {
    ...
    test("should return undefined when trying to retrieve a non-existing node by ID", () => {
    ...
    test("should throw an error upon trying to retrieve a non-existing edge by ID", () => {
      expect(graph.getEdgeById.bind(graph, undefined)).toThrowError("cannot retrieve edge with non-existing ID.");
      expect(graph.getEdgeById.bind(graph, loose_edge.getID())).toThrowError(
        "cannot retrieve edge with non-existing ID."
      );
      expect(graph.getEdgeById.bind(graph, out_edge.getID())).toThrowError(
        "cannot retrieve edge with non-existing ID."
      );
    });
    ...
    test("should report the number of directed edges currently in the graph", () => {
    ...
    test("should give you a random directed edge currently existing in the graph", () => {
    ...
    test("should give you a random undirected edge currently existing in the graph", () => {
     ...
     *
     */
    test("should throw an error retrieving an edge by Node IDs if node_a does not exist", () => {
      expect(graph.getDirEdgeByNodeIDs.bind(graph, undefined, node_b.getID())).toThrowError(
        "Cannot find edge. Node A does not exist (in graph)."
      );
    });

    test("should throw an error retrieving an edge by Node IDs if node_b does not exist", () => {
      expect(graph.getDirEdgeByNodeIDs.bind(graph, node_b.getID(), undefined)).toThrowError(
        "Cannot find edge. Node B does not exist (in graph)."
      );
    });

    test("should throw an error retrieving a non-existing edge between two valid graph nodes", () => {
  ...
  describe("A little more complex scenario with 4 nodes and 7 edges, mixed _mode", () => {
    let graph, n_a, n_b, n_c, n_d, node_vana, e_1, e_2, e_3, e_4, e_5, e_6, e_7;
     ...
     */
    test("should remove an existing undirected edge, updating graph and node stats", () => {
     ...
     * graph is still in MIXED _mode
     */
    test("should remove an existing directed edge, updating graph and node stats", () => {
      let graph_nr_nodes = graph.nrNodes(),
        graph_nr_dir_edges = graph.nrDirEdges(),
        graph_nr_und_edges = graph.nrUndEdges(),
        n_a_deg = n_a.deg,
     ...
     */
    test("should remove ALL undirected edges, bringing the graph into DIRECTED _mode", () => {
     ...
     */
    test("should remove ALL directed edges, bringing the graph into UNDIRECTED _mode", () => {
     ...
     */
    test("should throw an error when trying to delete outgoing edges of a foreign node", () => {
      expect(graph.deleteOutEdgesOf.bind(graph, node_vana)).toThrowError(
        "Cowardly refusing to delete edges of a foreign node."
      );
    });
     ...
     */
    test("should throw an error when trying to delete incoming edges of a foreign node", () => {
      expect(graph.deleteInEdgesOf.bind(graph, node_vana)).toThrowError(
        "Cowardly refusing to delete edges of a foreign node."
      );
    });
     ...
     */
    test("should throw an error when trying to delete directed edges of a foreign node", () => {
      expect(graph.deleteDirEdgesOf.bind(graph, node_vana)).toThrowError(
        "Cowardly refusing to delete edges of a foreign node."
      );
    });
     ...
     */
    test("should throw an error when trying to delete undirected edges of a foreign node", () => {
      expect(graph.deleteUndEdgesOf.bind(graph, node_vana)).toThrowError(
        "Cowardly refusing to delete edges of a foreign node."
      );
    });
     ...
     */
    test("should throw an error when trying to delete all edges of a foreign node", () => {
      expect(graph.deleteAllEdgesOf.bind(graph, node_vana)).toThrowError(
        "Cowardly refusing to delete edges of a foreign node."
      );
    });
  ...
  describe("Graph cloning - ", () => {
    ...
    test("should successfully clone an graph with a node plus nested feature vector", () => {
    ...
    test("should successfully clone an graph with two nodes and one undirected edge", () => {
      graph = new $G.BaseGraph("two_node_graph");
      let n_a = graph.addNodeByID("A");
      let n_b = graph.addNodeByID("B");
    ...
    test("should successfully clone an graph with two nodes and one directed edge", () => {
      graph = new $G.BaseGraph("two_node_graph");
      let n_a = graph.addNodeByID("A");
      let n_b = graph.addNodeByID("B");
     ...
     */
    test("should successfully clone a toy graph in explicit mode including weights", () => {
  ...
  describe("negative cycle checks - ", () => {
    ...
    let isWeightedSpy_e1, isWeightedSpy_e5;
    ...
    test("should have called isWeighted on e_1 and e_6 once each and returned true and false, respectively", () => {
    ...
    test("should correclty detect a graph with solely positive edges via hasNegativeEdge method", () => {
    ...
    test("should correclty detect a graph with solely positive edges via hasNegativeCycle method", () => {
      expect(graph.hasNegativeCycles(n_a)).toBe(false);
    });

    test("should correctly detect a graph with negative edges but no negative cycle", () => {
      e_2.setWeight(-2);
      expect(graph.hasNegativeCycles(n_a)).toBe(false);
    });
    ...
    test("should correctly detect a negative undirected edge as negative cycle", () => {
      e_2.setWeight(5);
    ...
    test("should correctly detect a negative cycle in a multi-component graph", () => {
  ...
  describe.skip("Graph PROJECTIONS - ", () => {
    ...
    describe("empty and trivial graphs - ", () => {
      ...
      test("should return the same directed graph if all edges were directed before", () => {
      ...
      test("should return a copy of the same directed graph if all edges were directed before", () => {
        let digraph_file = "./test/data/search_graph_pfs.json";
        let json = new JSONInput({ explicit_direction: true, directed: true, weighted: false });
        let digraph = json.readFromJSONFile(digraph_file);
        expect(digraph).toBeDefined();
