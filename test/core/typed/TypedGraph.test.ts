...
import { JSON_REC_PATH, JSON_TYPE_PATH } from "_/config/test_paths";

import { TypedEdge } from "@/core/typed/TypedEdge";
...
import { viaSharedPrefs } from "@/similarities/SimilarityCommons";
...
describe("TYPED GRAPH TESTS: ", () => {
  ...
  describe("Edges - ", () => {
    ...
    let graph: TypedGraph, a: ITypedNode, b: ITypedNode;
  ...
  describe("Typed graph node neighborhood tests - ", () => {
    ...
    const json = new JSONInput(),
      g: TypedGraph = json.readFromJSONFile(
        JSON_TYPE_PATH + "/office.json",
        new TypedGraph("office graph")
      ) as TypedGraph;
  ...
  describe("(sub)set expansion tests - ", () => {
    ...
    describe("get periphery @ K steps - ", () => {
      it("should not expand a negative number of steps", () => {
        expect(() => g.peripheryAtK(new Set([g.n(marie), g.n(tom)]), DIR.out, knows, { k: -1 })).toThrowError(
    ...
    describe("expand over K steps - ", () => {
      it("should not expand a negative number of steps", () => {
        expect(() => g.expandK(new Set([g.n(marie), g.n(tom)]), DIR.out, knows, { k: -1 })).toThrowError(
          "cowardly refusing to expand a negative number of steps."
        );
      });
      ...
      it("should expand K steps from a single node (OUT)", () => {
        ...
        const names = Array.from(expanse.set)
          .map(n => n.f("name"))
          .sort();
        // fs.writeFileSync('./data/output/marie_pfeffer_names.csv', names.join('\n'));
        const compareNames = fs.readFileSync("./data/results/marie_2_expand.csv").toString().trim().split(os.EOL);
    ...
    describe("expansion with correct frequencies (strengths / weights) - ", () => {
      let me: ITypedNode, myCompany: ITypedNode, myCountry: ITypedNode, employees: ExpansionResult;
      ...
      it("expand should correctly compute frequencies of employee skills", () => {
        ...
        const skillsFreqReadable = Array.from(skills.freq)
          .map(e => ({ name: e[0].f("name"), freq: e[1] }))
      ...
      it("expandK (=1) should correctly compute frequencies", () => {
        ...
        const skillsFreqReadable = Array.from(skills.freq)
          .map(e => ({ name: e[0].f("name"), freq: e[1] }))
          .sort((a, b) => b.freq - a.freq);
        const skillsK1FreqReadable = Array.from(skillsK1.freq)
          .map(e => ({ name: e[0].f("name"), freq: e[1] }))
          .sort((a, b) => b.freq - a.freq);
      ...
      it("expandK (=2) should correctly compute frequencies", () => {
        ...
        const friendsK2Readable = Array.from(friendsK2.freq)
          .map(e => ({ name: e[0].f("name"), freq: e[1] }))
          .sort((a, b) => b.freq - a.freq);
