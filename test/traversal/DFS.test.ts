...
import * as $DFS from "@/traversal/DFS";
...
describe("Basic GRAPH SEARCH Tests - Depth first search -", () => {
  ...
  describe("testing config preparation functions - ", () => {
    let prepForDFSSpy, prepForDFSVisitSpy;
    ...
    test("preprareDFSVisitStandardConfig should correctly instantiate a DFSConfig object", () => {
     ...
     * -> make it so!
     */
    test.skip("calling preprareDFSStandardConfig should also call prepareDFSVisitStandardConfig", () => {
      let config = $DFS.prepareDFSStandardConfig();
      expect(prepForDFSSpy).toHaveBeenCalledTimes(1);
      expect(prepForDFSVisitSpy).toHaveBeenCalledTimes(1);
    });

    test("preprareDFSStandardConfig should correctly instantiate a DFSConfig object", () => {
      let config = $DFS.prepareDFSStandardConfig();
  ...
  describe("testing DFS visit - empty graph and invalid dir_mode - ", () => {
    test("DFSVisit should throw an error upon trying to traverse an empty graph (INIT)", () => {
      let root = graph.getNodeById("A"),
        empty_graph = new $G.BaseGraph("iamemptygraph");

      expect($DFS.DFSVisit.bind($DFS.DFSVisit, empty_graph, root)).toThrowError(
        "Cowardly refusing to traverse graph without edges."
      );
    });

    test("DFSVisit should throw an error upon trying to traverse a graph with dir_mode set to INIT", () => {
      let root = graph.getNodeById("A"),
        config: $DFS.DFS_Config = {
          visit_result: {},
          dfs_visit_marked: {},
      ...
      expect($DFS.DFSVisit.bind($DFS.DFSVisit, graph, root, config)).toThrowError(
        "Cannot traverse a graph with dir_mode set to INIT."
      );
    });

    test("DFS should throw an error upon trying to traverse a blank graph (INIT)", () => {
      let root = graph.getNodeById("A"),
        empty_graph = new $G.BaseGraph("iamemptygraph");
    ...
    test("DFS should throw an error upon trying to traverse a graph with dir_mode set to INIT", () => {
      let root = graph.getNodeById("A"),
        config: $DFS.DFS_Config = {
          visit_result: {},
          dfs_visit_marked: {},
      ...
      expect($DFS.DFS.bind($DFS.DFS, graph, root, config)).toThrowError(
        "Cannot traverse a graph with dir_mode set to INIT."
      );
    });
  });
  ...
  describe("lookup DFS distance calculations - WEIGHTED EDGES - ", () => {
    test("should correctly compute lookup distance from node D - MIXED Mode", () => {
    ...
    test("should correctly compute lookup distance from node A - DIRECTED Mode", () => {
      jsonReader._config.weighted = true;
      let graph = jsonReader.readFromJSONFile(search_graph),
    ...
    test("should correctly compute lookup distance from node A - UNDIRECTED Mode", () => {
      jsonReader._config.weighted = true;
      let graph = jsonReader.readFromJSONFile(search_graph),
        root = graph.getNodeById("A"),
        config = $DFS.prepareDFSStandardConfig();
  ...
  describe("PFS_DFS graph traversal tests with edge weight ascending sort - ", () => {
    ...
    test("Should traverse search graph in correct order, ascending, root is A", () => {
      let root = graph.getNodeById("A"),
        config = $DFS.prepareDFSVisitStandardConfig();
    ...
    test("Should traverse search graph in correct order, ascending, root is B", () => {
    ...
    test("Should traverse search graph in correct order, DEscending, root is A", () => {
      let root = graph.getNodeById("A"),
        config = $DFS.prepareDFSVisitStandardConfig();
    ...
    test("Should traverse search graph in correct order, DEscending, root is B", () => {
      let root = graph.getNodeById("B"),
        config = $DFS.prepareDFSVisitStandardConfig();
     ...
     * NOW WITH WEIGHTS...
     */
    test("Should correctly compute weight distance with ascending sort function, root is A", () => {
      let root = graph.getNodeById("A"),
        config = $DFS.prepareDFSStandardConfig(),
        weight_dists = {};
    ...
    test("Should correctly compute weight distance with ascending sort function, root is B", () => {
      let root = graph.getNodeById("B"),
        config = $DFS.prepareDFSStandardConfig(),
        weight_dists = {};
    ...
    test("Should correctly compute weight distance with DEscending sort function, root is A", () => {
      let root = graph.getNodeById("A"),
        config = $DFS.prepareDFSStandardConfig(),
        weight_dists = {};
    ...
    test("Should correctly compute weight distance with DEscending sort function, root is B", () => {
      let root = graph.getNodeById("B"),
        config = $DFS.prepareDFSStandardConfig(),
        weight_dists = {};
