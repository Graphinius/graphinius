...
describe("GRAPH SEARCH Tests - Floyd-Warshall - ", () => {
  ...
  test("should refuse to compute FW APSP on empty graph", () => {
    ...
    expect($FW.FloydWarshallAPSP.bind($FW.FloydWarshallAPSP, empty_graph)).toThrowError(
  ...
  test("should refuse to compute FW array on empty graph", () => {
    let empty_graph = new $G.BaseGraph("iamempty");
    expect($FW.FloydWarshallArray.bind($FW.FloydWarshallArray, empty_graph)).toThrowError(
      "Cowardly refusing to traverse graph without edges."
    );
  });
  ...
  describe("FW on small search graph - ", () => {
    describe("computing distances in UNDIRECTED _mode - ", () => {
      ...
      test("should correctly compute distance matrix for graph, Array version", () => {
    ...
    test("75 nodes - FW with and without next should return same distance matrix", () => {
