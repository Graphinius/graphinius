...
import * as $BFS from "@/traversal/BFS";
...
describe("Basic GRAPH SEARCH Tests - Breadth first search - ", () => {
  let json: JSONInput, graph: $G.IGraph, stats: GraphStats, bfs_res: { [id: string]: $BFS.BFS_ResultEntry };
  ...
  describe("BFS on small search graph - ", () => {
    test("should refuse to traverse an empty graph", () => {
      ...
      expect($BFS.BFS.bind($BFS.BFS, empty_graph, root)).toThrowError(
    ...
    test("should refuse to traverse a graph with _mode set to INIT", () => {
      ...
      expect($BFS.BFS.bind($BFS.BFS, graph, root, config)).toThrowError(
        "Cannot traverse a graph with dir_mode set to INIT."
      );
    });

    test("should never expand a node when DIR mode is set to a meaningless value", () => {
      let root = graph.getNodeById("A"),
        config = $BFS.prepareBFSStandardConfig();
  ...
  describe("PFS_BFS graph traversal tests with edge weight ascending sort - ", () => {
    ...
    test("Should traverse search graph in correct order, ascending, root is D", () => {
      let root = graph.getNodeById("D"),
        config = $BFS.prepareBFSStandardConfig();
    ...
    test("Should traverse search graph in correct order, DEscending, root is D", () => {
      let root = graph.getNodeById("D"),
        config = $BFS.prepareBFSStandardConfig();
