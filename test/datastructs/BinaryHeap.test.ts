...
describe('BINARY HEAP PRIVATE METHOD TESTS', () => {
	...
	/**
	 * Checks the position of a certain value in the underlying array
	 */
	it('Checks that every node has at least position 0 (for INTs)', () => {
		binHeap = new $BH.BinaryHeap();
		let i = 0;
	...
	/**
	 * Do the same, but with floats & memorizing
	 */
	it('Checks that every node has at least position 0 (for INTs)', () => {
		binHeap = new $BH.BinaryHeap();
		let i = 0;
		...
		while (i < 100) {
			map[i] = Math.random()*Number.MAX_SAFE_INTEGER;
			binHeap.insert(map[i++]);
		}

		while (--i) {
			expect((<any>binHeap).getNodePosition(map[i])).toBeGreaterThanOrEqual(0);
		}
	});
