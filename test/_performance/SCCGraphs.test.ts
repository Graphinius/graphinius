...
import { CSV_PERF_PATH, OUTPUT_PATH } from "_/config/test_paths";

import { CSVInput, ICSVInConfig } from "@/io/input/CSVInput";
import { Logger } from "@/utils/Logger";
import { IGraph } from "@/core/base/BaseGraph";
import { DegreeCentrality, DegreeDistribution } from "@/centralities/Degree";
import { BFS } from "@/traversal/BFS";
import { DFS } from "@/traversal/DFS";
import { PFS } from "@/traversal/PFS";
import { Pagerank, PagerankRWConfig } from "@/centralities/Pagerank";
import { SimplePerturber } from "@/perturbation/SimplePerturbations";
...
describe("SCC Tests - ", () => {
  ...
  let SCCGraphs: Map<string, IGraph> = new Map(),
  ...
  beforeAll(() => {
    SCCFiles.forEach(scc_file => {
      ...
      const toc = +new Date();
      logger.log(
        `Reading in graph ${scc_file} with |V|=${graph.nrNodes()} and |E|=${graph.nrUndEdges()} took ${toc - tic} ms.`
      );
    });
  ...
  it(`Degree distributions`, () => {
    SCCGraphs.forEach(graph => {
      ...
      const toc = +new Date();
      logger.log(
        `Degree distribution for graph with |V|=${graph.nrNodes()} and |E|=${graph.nrUndEdges()} took ${toc - tic} ms.`
      );
    });
  });
  ...
  [BFS, DFS, PFS].forEach(traversal => {
    it(`${traversal.name}`, () => {
      SCCGraphs.forEach(graph => {
        ...
        const toc = +new Date();
        logger.log(
          `${traversal.name} for graph with |V|=${graph.nrNodes()} and |E|=${graph.nrUndEdges()} took ${toc - tic} ms.`
        );
      });
    });
  });
  ...
  it(`Pagerank (unweighted)`, () => {
    SCCGraphs.forEach(graph => {
      ...
      logger.log(
        `Pagerank (UNweighted) on graph with |V|=${graph.nrNodes()} and |E|=${graph.nrUndEdges()} took ${toc - tic} ms.`
      );
    });
  });
  ...
  it("Perturbation - Adding nodes with edges", () => {
    ...
    SCCGraphs.forEach(graph => {
      ...
      const toc = +new Date();
      logger.log(
        `Adding ${nrAddNodes} nodes & introducing new edges with p=${probEdge} on graph with |V|=${graph.nrNodes()} and |E|=${graph.nrUndEdges()} took ${
          toc - tic
        } ms.`
      );
    });
  ...
  it("Perturbation - Deleting nodes & edges", () => {
    SCCGraphs.forEach(graph => {
      ...
      const toc = +new Date();
      logger.log(
        `Cutting nodes in half on graph with |V|=${graph.nrNodes()} and |E|=${graph.nrUndEdges()} took ${toc - tic} ms.`
      );
    });
  });
});
