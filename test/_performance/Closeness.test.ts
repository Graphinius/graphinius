...
import { ClosenessCentrality } from "@/centralities/Closeness";
...
const sn_graph_300 = csv.readFromEdgeListFile(`${CSV_SN_PATH}/social_network_edges_300.csv`),
...
describe("Performance tests - ", () => {
  [sn_graph_300].forEach(sn_graph => {
    test("should run the closeness centrality on social networks, should be same as FW", () => {
      ...
      let toc = +new Date();
      logger.log(
        `Closeness on graph of |V|=${sn_graph.nrNodes()} and |E|=${sn_graph.nrUndEdges()} using FW took ${
          toc - tic
        } ms to finish`
      );
      ...
      toc = +new Date();
      logger.log(
        `Closeness on graph of |V|=${sn_graph.nrNodes()} and |E|=${sn_graph.nrUndEdges()} using PFS took ${
          toc - tic
        } ms to finish`
      );
  ...
  test("should run the closeness centrality on a ~1k social networks, just FW", () => {
    ...
    let toc = +new Date();
    logger.log(
      `Closeness on graph of |V|=${sn_graph_1K.nrNodes()} and |E|=${sn_graph_1K.nrUndEdges()} using FW took ${
        toc - tic
      } ms to finish`
    );
  });
