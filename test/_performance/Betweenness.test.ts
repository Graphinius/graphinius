...
import { CSV_SN_PATH, RES_CENT_PATH } from "_/config/test_paths";

import { Brandes } from "@/centralities/Brandes";
...
const socialNet300 = "social_network_edges_300",
...
describe("Betweenness centrality performance tests - ", () => {
  describe("comparison to PFS", () => {
    [false, true].forEach(normalize => {
      it(`compares runtimes of BrandesForWeighted to PFS based Brandes, normalize: ${normalize}`, () => {
        ...
        logger.log(
          `Running on graph of ${sn_300_graph.nrNodes()} nodes and ${
            sn_300_graph.nrDirEdges() + sn_300_graph.nrUndEdges()
          } edges, UNnormalized mode:`
        );
  ...
  describe("(UN)weighted social networks", () => {
    [sn_300_graph_d].forEach(graph => {
      // sn_1K_graph_d, sn_20K_graph_d
      test(`Runtime of Brandes (UNweighted) on graph ${graph.label}:`, () => {
        ...
        let endBU = +new Date();
        logger.log(
          `runtime of Brandes, UNweighted, on a ${graph.nrNodes()} nodes and ${
            graph.nrDirEdges() + graph.nrUndEdges()
          } edges social network: ${endBU - startBU} ms.`
        );
    ...
    [sn_300_graph_dw].forEach(graph => {
      // sn_1K_graph_dw, sn_20K_graph_dw
      test(`Runtime of Brandes (Weighted) on graph ${graph.label}:`, () => {
        ...
        let endBW = +new Date();
        logger.log(
          `runtime of Brandes, weighted, on a ${graph.nrNodes()} nodes and ${
            graph.nrDirEdges() + graph.nrUndEdges()
          } edges social network: ${endBW - startBW} ms.`
        );
