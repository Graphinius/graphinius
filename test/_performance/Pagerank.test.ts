...
import { CSV_EGO_PATH, CSV_SN_PATH, RES_CENT_PATH, JSON_REC_PATH } from "_/config/test_paths";
import { Pagerank } from "@/centralities/Pagerank";
...
const sn_300_file = `social_network_edges_300.csv`,
...
describe("Page Rank Random Walk performance tests on actual (small) social graphs - ", () => {
  [sn_300_file, sn_1K_file].forEach(graph_file => {
    //sn_300_file, sn_1K_file, sn_20K_file
    test("should calculate the PR via Random Walk for graphs of realistic size", () => {
      ...
      let toc = +new Date();
      logger.log(
        `Single-Thread JS PageRank (Arrays) on graph of |V|=${sn_graph.nrNodes()} and |E|=${sn_graph.nrUndEdges()} took ${
          toc - tic
        } ms.`
      );
...
describe("Page Rank Random Walk on ego graphs + comparison to networkx - ", () => {
  fs.readdirSync(CSV_EGO_PATH).forEach(graph_file => {
    test("should calculate the PR via Random Walk for EGO graphs of realistic size", () => {
      ...
      let toc = +new Date();
      logger.log(
        `PageRank for graph of |V|=${sn_graph.nrNodes()} and |E|=${sn_graph.nrUndEdges()} took ${toc - tic} ms.`
      );

      let controlFileName = `${RES_CENT_PATH}/pagerank/comparison_ego_graphs/pagerank_numpy_ego_network_v_${sn_graph.nrNodes()}_e_${
        sn_graph.nrUndEdges() * 2
      }.json`;
