import { CSV_CENT_PATH, CSV_DATA_PATH, CSV_SN_PATH, JSON_DATA_PATH } from "_/config/test_paths";

import * as $G from "@/core/base/BaseGraph";
import { CSVInput, ICSVInConfig } from "@/io/input/CSVInput";
import { JSONInput, IJSONInConfig } from "@/io/input/JSONInput";
import * as $CC from "@/centralities/Closeness";
...
describe("Closeness Centrality Tests", () => {
  ...
  test("Testing on single node graph", () => {
    ...
    expect(CC.getCentralityMap.bind(CC.getCentralityMap, graph_1)).toThrowError(
      "Cowardly refusing to traverse graph without edges."
    );
    expect(CC.getCentralityMapFW.bind(CC.getCentralityMapFW, graph_1)).toThrowError(
      "Cowardly refusing to traverse graph without edges."
    );
