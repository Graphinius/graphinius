 ...
 * @param cfg object
 */
export function simSource(
  algo: Function,
  s: string,
  t: $I.SetOfSets,
  cfg: $I.SimilarityConfig = {}
): $I.SimilarityResult {
...
export function knnNodeArray(
  ...
  for (let node of Object.keys(s)) {
    const topKEntries: $I.SimilarityEntry[] = simSource(algo, node, s, {
      knn: cfg.knn || 1,
      sort: cfg.sort,
    });
    topKEntries.forEach(e => {
      ...
      if (
        !cfg.dup &&
        ((dupes[e.from] && dupes[e.from][e.to]) ||
          (dupes[e.to] && dupes[e.to][e.from]))
      ) {
        return;
      }
      topK.push(e);
 ...
 */
export function viaSharedPrefs(
  g: TypedGraph,
  algo: Function,
  cfg: $I.SimPerSharedPrefConfig
) {
  const sort = cfg.sort || sortFuncs.desc;
  ...
  for (let [t1Name, t1Node] of t1Set.entries()) {
    for (let [t2Name, t2Node] of t2Set.entries()) {
      let prefSet1: Set<ITypedNode>, prefSet2: Set<ITypedNode>;
