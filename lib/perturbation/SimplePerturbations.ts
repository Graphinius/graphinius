...
export interface ISimplePerturber {
  ...
  createEdgesProb(
    probability: number,
    directed?: boolean,
    setOfNodes?: { [key: string]: $N.IBaseNode }
  ): void;
  createEdgesSpan(
    min: number,
    max: number,
  ...
  addNodesPercentage(
    percentage: number,
    config?: NodeDegreeConfiguration
  ): void;
...
class SimplePerturber implements ISimplePerturber {
  ...
  deleteNodesPercentage(percentage: number): void {
    ...
    let nr_nodes_to_delete = Math.ceil(
      (this._graph.nrNodes() * percentage) / 100
    );
  ...
  deleteUndEdgesPercentage(percentage: number): void {
    ...
    let nr_edges_to_delete = Math.ceil(
      (this._graph.nrUndEdges() * percentage) / 100
    );
  ...
  deleteDirEdgesPercentage(percentage: number): void {
    ...
    let nr_edges_to_delete = Math.ceil(
      (this._graph.nrDirEdges() * percentage) / 100
    );
  ...
  deleteNodesAmount(amount: number): void {
    ...
    for (
      let nodeID = 0,
        randomNodes = this._graph.pickRandomProperties(
          this._graph.getNodes(),
          amount
        );
      nodeID < randomNodes.length;
      nodeID++
    ) {
  ...
  deleteUndEdgesAmount(amount: number): void {
    ...
    for (
      let edgeID = 0,
        randomEdges = this._graph.pickRandomProperties(
          this._graph.getUndEdges(),
          amount
        );
      edgeID < randomEdges.length;
      edgeID++
    ) {
   ...
   */
  addUndEdgesPercentage(percentage: number): void {
    let nr_und_edges_to_add = Math.ceil(
      (this._graph.nrUndEdges() * percentage) / 100
    );
   ...
   */
  addDirEdgesPercentage(percentage: number): void {
    let nr_dir_edges_to_add = Math.ceil(
      (this._graph.nrDirEdges() * percentage) / 100
    );
  ...
  addEdgesAmount(amount: number, config?: $E.BaseEdgeConfig): void {
    ...
    while (amount > 0) {
      ...
      if (node_a.hasEdgeID(edge_id)) {
         ...
         */
        this._graph.addEdgeByID(edge_id, node_a, node_b, {
          directed: direction,
        });
        --amount;
      }
  ...
  private createEdgesByConfig(
    config: NodeDegreeConfiguration,
    new_nodes: { [key: string]: $N.IBaseNode }
  ) {
    let degree, min_degree, max_degree, deg_probability;

    if (
      config.und_degree != null ||
      ...
      if ((degree = config.und_degree) != null) {
        ...
        (min_degree = config.min_und_degree) != null &&
      ...
      if ((degree = config.dir_degree)) {
        ...
        (min_degree = config.min_dir_degree) != null &&
    ...
    for (node_a in new_nodes) {
      for (node_b in all_nodes) {
        if (node_a !== node_b && Math.random() <= probability) {
          edge_id =
            all_nodes[node_a].getID() + "_" + all_nodes[node_b].getID() + dir;
