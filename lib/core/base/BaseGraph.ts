...
class BaseGraph implements IGraph {
  ...
  getStats(): GraphStats {
    return {
      ...
      density_und:
        (2 * this._nr_und_edges) / (this._nr_nodes * (this._nr_nodes - 1)),
    };
  }
  ...
  getDirEdgeByNodeIDs(node_a_id: string, node_b_id: string) {
    ...
    throw new Error(
      `Cannot find edge. There is no edge between Node ${node_a_id} and ${node_b_id}.`
    );
  }
  ...
  getUndEdgeByNodeIDs(node_a_id: string, node_b_id: string) {
    ...
    for (let i = 0; i < edges_und_keys.length; i++) {
      ...
      let b: string;
      edge.getNodes().a.getID() == node_a_id
        ? (b = edge.getNodes().b.getID())
        : (b = edge.getNodes().a.getID());
      if (b == node_b_id) {
        return edge;
      }
    }
  ...
  addEdge(edge: IBaseEdge): IBaseEdge {
    ...
    if (
      !this.hasNodeID(node_a.getID()) ||
      !this.hasNodeID(node_b.getID()) ||
      this._nodes[node_a.getID()] !== node_a ||
      this._nodes[node_b.getID()] !== node_b
    ) {
  ...
  cloneSubGraphStructure(root: IBaseNode, cutoff: Number): IGraph {
    ...
    let bfsNodeUnmarkedTestCallback = function (context: BFS_Scope) {
      if (config.result[context.next_node.getID()].counter > cutoff) {
        ...
        //This means we only add cutoff -1 nodes to the cloned graph, # of nodes is then equal to cutoff
  ...
  pickRandomProperty(propList): any {
    ...
    let randomPropertyName =
      tmpList[Math.floor(Math.random() * tmpList.length)];
