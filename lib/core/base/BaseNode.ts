...
class BaseNode implements IBaseNode {
   ...
   * @param config
   */
  constructor(protected _id: string, config: BaseNodeConfig = {}) {
    this._in_edges = {};
    this._out_edges = {};
    this._und_edges = {};
  ...
  hasEdge(edge: IBaseEdge): boolean {
    return (
      !!this._in_edges[edge.getID()] ||
      !!this._out_edges[edge.getID()] ||
      !!this._und_edges[edge.getID()]
    );
  }

  hasEdgeID(id: string): boolean {
    return (
      !!this._in_edges[id] || !!this._out_edges[id] || !!this._und_edges[id]
    );
  }
  ...
  allNeighbors(identityFunc?: Function): Array<NeighborEntry> {
    ...
    return $SU.mergeArrays(
      [this.prevNodes(), this.nextNodes(), this.connNodes()],
      identityFunc ||
        function (ne) {
          return identity++;
        }
