...
class TypedNode extends BaseNode implements ITypedNode {
  ...
  get stats(): TypedNodeStats {
    ...
    for (let type of Object.keys(this._typedAdjSets)) {
      ...
      result.typed_edges[type].ins = this._typedAdjSets[type].ins
        ? this._typedAdjSets[type].ins.size
        : 0;
      result.typed_edges[type].outs = this._typedAdjSets[type].outs
        ? this._typedAdjSets[type].outs.size
        : 0;
      result.typed_edges[type].conns = this._typedAdjSets[type].conns
        ? this._typedAdjSets[type].conns.size
        : 0;
    }
    return result;
  }
  ...
  unds(type: string): Set<string> {
    return this._typedAdjSets[type]
      ? this._typedAdjSets[type].conns
      : undefined;
  }
  ...
  all(type: string): Set<string> {
    ...
    if (this._typedAdjSets[type]) {
      this._typedAdjSets[type].ins &&
        result.add([...this._typedAdjSets[type].ins]);
      this._typedAdjSets[type].outs &&
        result.add([...this._typedAdjSets[type].outs]);
      this._typedAdjSets[type].conns &&
        result.add([...this._typedAdjSets[type].conns]);
    }
    return result;
  }
  ...
  private noEdgesOfTypeLeft(type: string): boolean {
    return (
      (!this._typedAdjSets[type].ins || !this._typedAdjSets[type].ins.size) &&
