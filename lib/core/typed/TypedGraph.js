...
class TypedGraph extends BaseGraph_1.BaseGraph {
    constructor(_label) {
        ...
        this._type = run_config_1.GENERIC_TYPES.GRAPH;
        this._typedNodes.set(run_config_1.GENERIC_TYPES.NODE, new Map());
        this._typedEdges.set(run_config_1.GENERIC_TYPES.EDGE, new Map());
    }
    get type() {
        return this._type;
    }
    ...
    deleteNode(node) {
        const id = node.getID(), type = node.type ? node.type.toUpperCase() : run_config_1.GENERIC_TYPES.NODE;
        if (!this._typedNodes.get(type)) {
    ...
    addEdge(edge) {
        ...
        const id = edge.getID();
        let type = run_config_1.GENERIC_TYPES.EDGE;
