...
class ComputeGraph implements IComputeGraph {
  ...
  checkNumericHandler() {
    if (!this._numeric || !this._numeric.matMul) {
      throw new Error(
        "Tensorflow & TF matMul function must be present for fast numeric computations."
      );
    }
  }
  ...
  nextArray(incoming: boolean = false): NextArray {
    ...
    for (let i = 0; i < this._g.nrNodes(); ++i) {
      ...
      for (let j = 0; j < this._g.nrNodes(); ++j) {
        next[i].push([]);
        next[i][j].push(
          i === j ? j : isFinite(adjDict[node_keys[i]][node_keys[j]]) ? j : null
        );
      }
    }
    return next;
  ...
  adjMatrix(): MinAdjacencyListArray {
    ...
    for (let i = 0; i < this._g.nrNodes(); ++i) {
      ...
      for (let j = 0; j < this._g.nrNodes(); ++j) {
        adjList[i].push(
          i === j ? 0 : isFinite(adjDict[node_keys[i]][node_keys[j]]) ? 1 : 0
        );
      }
    }
    return adjList;
    ...
    for (let i = 0; i < this._g.nrNodes(); ++i) {
      ...
      for (let j = 0; j < this._g.nrNodes(); ++j) {
        adjList[i].push(
          i === j
            ? self_dist
            : isFinite(adjDict[node_keys[i]][node_keys[j]])
            ? adjDict[node_keys[i]][node_keys[j]]
  ...
  adjListW(
    ...
    for (key in nodes) {
      let neighbors = incoming
        ? nodes[key].reachNodes().concat(nodes[key].prevNodes())
        : nodes[key].reachNodes();

      neighbors.forEach(ne => {
        cur_dist =
          adj_list_dict[key][ne.node.getID()] || Number.POSITIVE_INFINITY;
        cur_edge_weight = isNaN(ne.edge.getWeight())
          ? DEFAULT_WEIGHT
          : ne.edge.getWeight();
        ...
        if (cur_edge_weight < cur_dist) {
          ...
          if (incoming) {
            // we need to update the 'inverse' entry as well
  ...
  triadCount(directed = false): number {
    ...
    for (let n of nodes) {
      if (directed) {
        triangle_count +=
          (n.in_deg - n.self_in_deg) * (n.out_deg - n.self_out_deg);
      } else {
