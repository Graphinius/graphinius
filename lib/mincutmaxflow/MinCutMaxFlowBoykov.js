...
class MCMFBoykov {
    ...
    calculateCycle() {
        ...
        const smallTree_size = Object.keys(smallTree).length;
        const smallTree_ids = Object.keys(smallTree);
        for (let i = 0; i < smallTree_size; i++) {
            const node_id = smallTree_ids[i];
            const node = this._graph.getNodeById(node_id);
            if (!this._config.directed) {
                const undEdges = node.undEdges();
                const undEdges_size = Object.keys(undEdges).length;
                const undEdges_ids = Object.keys(undEdges);
                ...
                const outEdges_ids = Object.keys(node.outEdges());
                const outEdges_length = outEdges_ids.length;
                const inEdges_ids = Object.keys(node.inEdges());
                const inEdges_length = inEdges_ids.length;
    ...
    renameEdges(graph) {
        ...
        const edges_ids = Object.keys(edges);
        const edges_length = edges_ids.length;
        for (let i = 0; i < edges_length; i++) {
            const edge = edges[edges_ids[i]];
            const weight = edge.getWeight();
            graph.deleteEdge(edge);
            const node_a = edge.getNodes().a;
            const node_b = edge.getNodes().b;
    ...
    convertToDirectedGraph(uGraph) {
        const dGraph = new $G.BaseGraph(uGraph.label + "_directed");
        const nodes = uGraph.getNodes();
        const nodes_ids = Object.keys(nodes);
        const nodes_length = nodes_ids.length;
        for (let i = 0; i < nodes_length; i++) {
            const node = nodes[nodes_ids[i]];
            dGraph.addNodeByID(node.getID());
        }
        const edges = uGraph.getUndEdges();
        const edges_ids = Object.keys(edges);
        const edges_length = edges_ids.length;
        for (let i = 0; i < edges_length; i++) {
            const und_edge = edges[edges_ids[i]];
            const node_a_id = und_edge.getNodes().a.getID();
            const node_b_id = und_edge.getNodes().b.getID();
            const options = { directed: true, weighted: true, weight: und_edge.getWeight() };
    ...
    getPathToRoot(node) {
        ...
        const sink_id = this._sink.getID();
        const source_id = this._source.getID();
    ...
    getBottleneckCapacity() {
        ...
        for (let i = 0; i < path_length; i++) {
            const node_a = this._state.path[i];
    ...
    grow() {
        let nr_active_nodes = Object.keys(this._state.activeNodes).length;
        const active_nodes_ids = Object.keys(this._state.activeNodes);
        while (nr_active_nodes) {
            const activeNode = this._state.activeNodes[active_nodes_ids[0]];
            const edges = (this._state.tree[activeNode.getID()] == "S") ? activeNode.outEdges() : activeNode.inEdges();
            const edges_ids = Object.keys(edges);
            const edges_length = edges_ids.length;
            for (let i = 0; i < edges_length; i++) {
                const edge = edges[edges_ids[i]];
                const neighborNode = (this._state.tree[activeNode.getID()] == "S") ? edge.getNodes().b : edge.getNodes().a;
                if (edge.getWeight() <= 0) {
                    continue;
                }
                if (!(this._state.tree[neighborNode.getID()])) {
                    ...
                    let complete_path;
                    let nPath = this.getPathToRoot(neighborNode);
                    let aPath = this.getPathToRoot(activeNode);
                    const root_node_npath = nPath[nPath.length - 1];
    ...
    adoption() {
        ...
        while (orphans_size) {
            const orphan = this._state.orphans[orphans_ids[0]];
            delete this._state.orphans[orphan.getID()];
            orphans_ids.shift();
            --orphans_size;
            const edges = (this._state.tree[orphan.getID()] == "S") ? orphan.inEdges() : orphan.outEdges();
            const edge_ids = Object.keys(edges);
            const edge_length = edge_ids.length;
            let found = false;
            for (let i = 0; i < edge_length; i++) {
                ...
                if ((this._state.tree[orphan.getID()] == this._state.tree[neighbor.getID()]) && edge.getWeight()) {
                    const neighbor_root_path = this.getPathToRoot(neighbor);
                    const neighbor_root = neighbor_root_path[neighbor_root_path.length - 1];
