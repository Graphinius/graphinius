...
class MCMFBoykov implements IMCMFBoykov {
  ...
  constructor(
    ...
    if (!this._config.directed) {
      ...
      this._state.residGraph = this.convertToDirectedGraph(
        this._state.residGraph
      );
  ...
  calculateCycle() {
     ...
     */
    const smallTree =
      Object.keys(this._state.treeS).length <
      Object.keys(this._state.treeT).length
        ? this._state.treeS
        : this._state.treeT;
    ...
    for (let i = 0; i < smallTree_size; i++) {
      ...
      if (!this._config.directed) {
        ...
        for (let i = 0; i < undEdges_size; i++) {
          ...
          let neighbor: $N.IBaseNode =
            edge.getNodes().a.getID() == node.getID()
              ? edge.getNodes().b
              : edge.getNodes().a;
          // if (this.tree(neighbor) != this.tree(node)) {
          if (
            this._state.tree[neighbor.getID()] != this._state.tree[node.getID()]
          ) {
  ...
  renameEdges(graph: $G.IGraph) {
    ...
    for (let i = 0; i < edges_length; i++) {
      ...
      const new_edge = graph.addEdgeByID(
        node_a.getID() + "_" + node_b.getID(),
        node_a,
        node_b,
        options
      );
  ...
  convertToDirectedGraph(uGraph: $G.IGraph): $G.IGraph {
    ...
    for (let i = 0; i < edges_length; i++) {
      ...
      const options: $E.BaseEdgeConfig = {
        directed: true,
        weighted: true,
        weight: und_edge.getWeight(),
      };

      dGraph.addEdgeByID(
        node_a_id + "_" + node_b_id,
        dGraph.getNodeById(node_a_id),
        dGraph.getNodeById(node_b_id),
        options
      );
      dGraph.addEdgeByID(
        node_b_id + "_" + node_a_id,
        dGraph.getNodeById(node_b_id),
        dGraph.getNodeById(node_a_id),
        options
      );
  ...
  getPathToRoot(node: $N.IBaseNode) {
    ...
    while (node_id != sink_id && node_id != source_id) {
      if (this._state.parents[node_id] == null) {
        // this happens when the root of this path is a free node
        return path_root;
      }
  ...
  getBottleneckCapacity() {
    ...
    min_capacity = this._state.residGraph
      .getEdgeById(
        this._state.path[0].getID() + "_" + this._state.path[1].getID()
      )
      .getWeight();
    ...
    for (let i = 0; i < path_length; i++) {
      ...
      const edge = this._state.residGraph.getEdgeById(
        node_a.getID() + "_" + node_b.getID()
      );
  ...
  grow() {
    ...
    while (nr_active_nodes) {
      ...
      for (let i = 0; i < edges_length; i++) {
        ...
        const edge: $E.IBaseEdge = edges[edges_ids[i]];
        const neighborNode: $N.IBaseNode =
          this._state.tree[activeNode.getID()] == "S"
            ? edge.getNodes().b
            : edge.getNodes().a;
        ...
        if (!this._state.tree[neighborNode.getID()]) {
          ...
          this._state.tree[neighborNode.getID()] !=
          this._state.tree[activeNode.getID()]
        ) {
          // constructing path
  ...
  augmentation() {
    ...
    for (let i = 0; i < this._state.path.length - 1; i++) {
      const node_a = this._state.path[i],
        node_b = this._state.path[i + 1];
      // let edge = this._state.residGraph.getEdgeByNodeIDs(node_a.getID(), node_b.getID());
      let edge = this._state.residGraph.getEdgeById(
        node_a.getID() + "_" + node_b.getID()
      );
      // let reverse_edge = this._state.residGraph.getEdgeByNodeIDs(node_b.getID(), node_a.getID());
      const reverse_edge = this._state.residGraph.getEdgeById(
        node_b.getID() + "_" + node_a.getID()
      );

      // update the residual capacity in the graph
      this._state.residGraph
        .getEdgeById(edge.getID())
        .setWeight(edge.getWeight() - min_capacity);
      this._state.residGraph
        .getEdgeById(reverse_edge.getID())
        .setWeight(reverse_edge.getWeight() + min_capacity);
      // for all saturated edges
      edge = this._state.residGraph.getEdgeById(edge.getID());
      if (!edge.getWeight()) {
        if (
          this._state.tree[node_a.getID()] == this._state.tree[node_b.getID()]
        ) {
  ...
  adoption() {
    ...
    while (orphans_size) {
      ...
      const edges: { [k: string]: $E.IBaseEdge } =
        this._state.tree[orphan.getID()] == "S"
          ? orphan.inEdges()
          : orphan.outEdges();
      ...
      for (let i = 0; i < edge_length; i++) {
        ...
        if (
          this._state.tree[orphan.getID()] ==
            this._state.tree[neighbor.getID()] &&
          edge.getWeight()
        ) {
          const neighbor_root_path: Array<$N.IBaseNode> = this.getPathToRoot(
            neighbor
          );
          const neighbor_root: $N.IBaseNode =
            neighbor_root_path[neighbor_root_path.length - 1];
          // check for root either source or sink
          if (
            neighbor_root.getID() == this._sink.getID() ||
            neighbor_root.getID() == this._source.getID()
          ) {
      ...
      for (let i = 0; i < edge_length; i++) {
        ...
        let neighbor: $N.IBaseNode =
          this._state.tree[orphan.getID()] == "S"
            ...
            : edge.getNodes().b;
        if (
          this._state.tree[orphan.getID()] == this._state.tree[neighbor.getID()]
        ) {
          // set neighbor active
          if (edge.getWeight()) {
