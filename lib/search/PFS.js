...
var $E = require("../core/base/BaseEdge");
...
function PFS(graph, v, config) {
    ...
    if (graph.getMode() === interfaces_1.GraphMode.INIT) {
        throw new Error('Cowardly refusing to traverse graph without edges.');
    }
    if (dir_mode === interfaces_1.GraphMode.INIT) {
    ...
    while (scope.OPEN_HEAP.size()) {
        ...
        if (dir_mode === interfaces_1.GraphMode.MIXED) {
            scope.adj_nodes = scope.current.node.reachNodes();
        }
        else if (dir_mode === interfaces_1.GraphMode.UNDIRECTED) {
            scope.adj_nodes = scope.current.node.connNodes();
        }
        else if (dir_mode === interfaces_1.GraphMode.DIRECTED) {
...
function preparePFSStandardConfig() {
    var config = {
        ...
        dir_mode: interfaces_1.GraphMode.MIXED,
        goal_node: null,
