...
class EMEBoykov implements IEMEBoykov {
  ...
  calculateCycle() {
    ...
    while (success) {
      ...
      for (let i = 0; i < this._labels.length; i++) {
        ...
        let source: $N.IBaseNode = this._state.expansionGraph.getNodeById(
          "SOURCE"
        );
        ...
        MinCut = new $MC.MCMFBoykov(
          this._state.expansionGraph,
          source,
          sink,
          mincut_options
        );
  ...
  constructGraph(): $G.IGraph {
    ...
    for (let i = 0; i < node_ids.length; i++) {
      ...
      let edge_options: $E.BaseEdgeConfig = {
        directed: true,
        weighted: true,
        weight: 0,
      };
      // add edge to source
      edge_options.weight = this._dataTerm(
        this._state.activeLabel,
        this._graph.getNodeById(node.getID()).getLabel()
      );
      let edge_source: $E.IBaseEdge = graph.addEdgeByID(
        node.getID() + "_" + source.getID(),
        node,
        source,
        edge_options
      );
      let edge_source_reverse: $E.IBaseEdge = graph.addEdgeByID(
        source.getID() + "_" + node.getID(),
        source,
        node,
        edge_options
      );
      ...
      // add edge to sink
      edge_options.weight =
        node.getLabel() == this._state.activeLabel
          ? Infinity
          : this._dataTerm(
              node.getLabel(),
              this._graph.getNodeById(node.getID()).getLabel()
            );
      let edge_sink: $E.IBaseEdge = graph.addEdgeByID(
        node.getID() + "_" + sink.getID(),
        node,
        sink,
        edge_options
      );
      let edge_sink_source: $E.IBaseEdge = graph.addEdgeByID(
        sink.getID() + "_" + node.getID(),
        sink,
        node,
        edge_options
      );
    ...
    for (let i = 0; i < edges_length; i++) {
      ...
      if (node_p.getLabel() == node_q.getLabel()) {
        ...
        edge_options.weight = this._interactionTerm(
          node_p.getLabel(),
          this._state.activeLabel
        );
        graph.deleteEdge(edge);
        graph.addEdgeByID(
          node_p.getID() + "_" + node_q.getID(),
          node_p,
          node_q,
          edge_options
        );
        graph.addEdgeByID(
          node_q.getID() + "_" + node_p.getID(),
          node_q,
          node_p,
          edge_options
        );
      ...
      // add auxiliary node
      let node_aux: $N.IBaseNode = graph.addNodeByID(
        "aux_" + node_p.getID() + "_" + node_q.getID()
      );
      ...
      let edge_p_aux: $E.IBaseEdge = graph.addEdgeByID(
        node_p.getID() + "_" + node_aux.getID(),
        node_p,
        node_aux,
        edge_options
      );
      let edge_p_aux_reverse: $E.IBaseEdge = graph.addEdgeByID(
        node_aux.getID() + "_" + node_p.getID(),
        node_aux,
        node_p,
        edge_options
      );
      ...
      let edge_aux_q: $E.IBaseEdge = graph.addEdgeByID(
        node_aux.getID() + "_" + node_q.getID(),
        node_aux,
        node_q,
        edge_options
      );
      let edge_aux_q_reverse: $E.IBaseEdge = graph.addEdgeByID(
        node_q.getID() + "_" + node_aux.getID(),
        node_q,
        node_aux,
        edge_options
      );
      ...
      let edge_aux_sink: $E.IBaseEdge = graph.addEdgeByID(
        node_aux.getID() + "_" + sink.getID(),
        node_aux,
        sink,
        edge_options
      );
      let edge_aux_sink_reverse: $E.IBaseEdge = graph.addEdgeByID(
        sink.getID() + "_" + node_aux.getID(),
        sink,
        node_aux,
        edge_options
      );
  ...
  prepareEMEStandardConfig(): EMEConfig {
    ...
    let interactionTerm: EnergyFunctionTerm = function (
      label_a: string,
      label_b: string
    ) {
    ...
    let dataTerm: EnergyFunctionTerm = function (
      label: string,
      observed: string
    ) {
