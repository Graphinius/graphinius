...
class JSONInput {
    ...
    readFromJSON(json) {
        ...
        for (let node_id in json.data) {
            ...
            let label = json.data[node_id][interfaces_1.labelKeys.label];
            if (label) {
                node.setLabel(label);
            }
            if (features = json.data[node_id][interfaces_1.labelKeys.features]) {
                node.setFeatures(features);
            }
            if (coords_json = json.data[node_id][interfaces_1.labelKeys.coords]) {
                coords = {};
