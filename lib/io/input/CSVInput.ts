...
class CSVInput implements ICSVInput {
  ...
  constructor(config: ICSVInConfig = {}) {
    this._config = {
      ...
      direction_mode:
        config.direction_mode != null ? config.direction_mode : false,
  ...
  readFromAdjacencyList(input: Array<string>, graph_name: string): $G.IGraph {
    ...
    for (let idx in input) {
      let line = input[idx],
        elements = this._config.separator.match(/\s+/g)
          ? line.match(/\S+/g)
          : line.replace(/\s+/g, "").split(this._config.separator),
        node_id = elements[0],
        node: $N.IBaseNode,
      ...
      node = graph.hasNodeID(node_id)
        ? graph.getNodeById(node_id)
        : graph.addNodeByID(node_id);

      for (let e = 0; e < edge_array.length; ) {
        if (
          this._config.explicit_direction &&
          (!edge_array || edge_array.length % 2)
        ) {
          throw new Error(
            "Every edge entry has to contain its direction info in explicit mode."
          );
        }
        target_node_id = edge_array[e++];

        target_node = graph.hasNodeID(target_node_id)
          ? graph.getNodeById(target_node_id)
          : graph.addNodeByID(target_node_id);
        ...
        dir_char = this._config.explicit_direction
          ? edge_array[e++]
          : this._config.direction_mode
          ? "d"
          : "u";

        if (dir_char !== "d" && dir_char !== "u") {
          throw new Error(
            "Specification of edge direction invalid (d and u are valid)."
          );
        }
        directed = dir_char === "d";
        ...
        if (
          graph.hasEdgeID(edge_id) ||
          (!directed && graph.hasEdgeID(edge_id_u2))
        ) {
          // The completely same edge should only be added once...
          continue;
        } else {
          edge = graph.addEdgeByID(edge_id, node, target_node, {
            directed: directed,
          });
        }
      }
  ...
  readFromEdgeList(
    ...
    for (let idx in input) {
      ...
      edge_weight = this._config.weighted
        ? isNaN(parse_weight)
          ? DEFAULT_WEIGHT
          : parse_weight
        : null;
