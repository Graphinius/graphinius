...
gulp.task('test-partition', function () {
	return gulp.src(paths.tests_partition, {read: false})
					.pipe(mocha( mocha_options ));
});
...
gulp.task('watch-partition', function () {
	gulp.watch(paths.typescripts, ['test-partition']);
});
