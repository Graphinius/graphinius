    ...
    KLPartitioning.prototype.updateCosts = function (swap_ge) {
        ...
        [swap_ge.source, swap_ge.target].forEach(function (source) {
            ...
            source.allNeighbors().forEach(function (ne) {
                ...
                var gain_entry = _this._gainsHash.get(gain_id);
                if (!gain_entry) {
                    logger.log("No heap entry found for (" + gain_id + ")");
                    return;
                }
                _this._gainsHeap.remove(gain_entry);
                var edge_weight = _this._config.weighted ? _this._adjList[source_id][target_id] : DEFAULT_WEIGHT;
                var same_part = partitioning.nodePartMap.get(source_id) === partitioning.nodePartMap.get(target_id);
